...
The \emph{Fourier Transform} takes a time domain signal and represents it in the frequency domain. Given a complex signal that is mixed from multiple sinusoidal signals (with different frequencies), the Fourier Transform helps disclose the frequency of each of the sinusoidal signals. One key assumption for Fourier Transform is that any complex signal is actually a linear combination of multiple simple sinusoidal signals, e.g., 
$$x(t) = a_0 \cdot x_0(t) + a_1 \cdot x_1(t) + \cdots + a_n \cdot x_n(t),$$
where $x(t)$ is a complex signal, the series $x_i(t)$ are simple sinusoidal signals, and $a_i$ is the coefficient of $x_i(t)$. The Fourier Transform simply reveals the value of each $a_i$.

\subsection{Sampling}
For Discrete Fourier Transform, for the time series of a signal, we have to sample it in the time domain to get an array of values. There are two key factors that affects the resulted array: sampling frequency $f_s$ and duration $T_s$ of the signal in sampling. The higher sample frequency, the more samples we can get in 1 second. The longer duration, the more samples we can get in total.
...
To achieve better frequency resolution, we can either decrease $f_s$ or increase $N$. Increasing $N$ means we will take more samples at given sampling frequency $f_s$. Given any $f_s$, the more samples we have, the better frequency resolution can be characterized. However, decreasing $f_s$ is a bit counter intuitive. We should notice that given total samples $N$, a smaller $f_s$ means longer time in signal to get total $N$ samples. Therefore, no matter decreasing $f_s$ or  increasing $N$ , fundamentally both requires the signal lasts for longer time for collecting samples.
